<?php
namespace danixl\fac\utils\floatingtext;
...
class FTManager {
    ...
    public function updateFloatingTextParticle(Player $player, $newLevelName) {
        $lvlName = $player->getLevel()->getName();
        if($lvlName !== $newLevelName) {
            if(isset($this->particle[$newLevelName])) {
                foreach($this->particle[$newLevelName] as $k => $v) {
                    $player->getLevel()->addParticle($this->particle[$newLevelName][$k], [$player]); // ADDS NEW FT
                }
            }
        }
    }

    public function updateFloatingTextByLevel(Player $player, $newLevelName) {
        if(isset($this->spawnedTo[$player->getId()])) {
            $oldLvlName = $this->spawnedTo[$player->getId()];
            if($oldLvlName !== $newLevelName) {
                unset($this->spawnedTo[$player->getId()]);
                $this->spawnedTo[$player->getId()] = $newLevelName;
                if(isset($this->particle[$newLevelName])) {
                    foreach($this->particle[$newLevelName] as $k => $v) {
                        $player->getLevel()->addParticle($this->particle[$newLevelName][$k], [$player]); // ADDS NEW FT
                    }
                ...
                if(isset($this->particle[$oldLvlName])) {
                    foreach($this->particle[$oldLvlName] as $k => $v) {
                        ...
                        $player->getLevel()->addParticle($this->particle[$newLevelName][$k], [$player]);
                        $this->particle[$newLevelName][$k]->setInvisible(false);
                    }
                }
            }
        }
    ...
    public function editFT(Player $player, ?array $data): void {
        ...
        if(isset($this->ftEditor[$player->getName()])) {
            ...
            $this->updateFloatingTextByLevel($player, $level);
